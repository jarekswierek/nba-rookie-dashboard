...
class TestEmpty:
    def test_no_gaps_returns_empty(self) -> None:
        assert detect_context_events([], _stats(total_games=20)).events == []

    def test_zero_total_games_returns_empty(self) -> None:
        gap = _gap(1, 3)
        assert detect_context_events([gap], _stats(total_games=0)).events == []


class TestSingleGap:
    def test_short_ended_gap_yields_return(self) -> None:
        gap = _gap(5, 7)  # length 3
        result = detect_context_events([gap], _stats(total_games=20))
...
class TestExtendedAbsence:
    def test_long_ended_gap_yields_both_events(self) -> None:
        ...
        result = detect_context_events([gap], _stats(total_games=25))
...
class TestMultipleGaps:
    def test_only_latest_gap_yields_return_or_absent(self) -> None:
        ...
        result = detect_context_events(gaps, _stats(total_games=25))
    ...
    def test_unsorted_input_produces_same_result(self) -> None:
        ...
        sorted_result = detect_context_events(gaps_sorted, _stats(total_games=25))
        shuffled_result = detect_context_events(gaps_shuffled, _stats(total_games=25))
